...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_vel_temperature = 0.15  # A slight increase to boost forward movement
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Revised Upright reward with reduced scaling
    ...
    upright_temperature = 0.1  # Reduced temperature to minimize overpowering
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 1.0))
    
    # Revised Stability Reward for more weight on smooth motion
    ang_velocity = root_states[:, 10:13]
    stability_temperature = 0.3  # Increased temperature for emphasis
    ...
    total_reward = 1.5 * forward_vel_reward + 0.1 * upright_reward + 1.0 * stability_reward
