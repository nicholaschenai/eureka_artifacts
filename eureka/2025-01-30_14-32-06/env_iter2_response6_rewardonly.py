...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_vel_temperature = 0.1  # Adjusted temperature for forward velocity
    forward_vel_reward = torch.clamp(torch.exp(forward_vel_temperature * forward_vel) - 1.0, 0.0, 2.0)  # Clamped for better scaling
    ...
    # Apply scaling and transformation with reset
    upright_temperature = 1.5  # Significantly increased temperature to enhance differentiation
    upright_reward = torch.exp(-upright_temperature * torch.abs(dot_prod_up - 1.0))  # Negatively transformed

    # Total reward is a weighted sum of the components
    total_reward = 1.0 * forward_vel_reward + 1.0 * upright_reward
