...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Improved Reward for forward velocity
    forward_vel_temperature = 0.15  # Increased for more reward density
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Dynamic Upright Reward considering pitch and roll if accessible.
    ...
    upright_temperature = 0.2  # Lowered to keep balance but not over-encourage
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 0.8))  # Slightly adjusted baseline

    # Reworked Stability Reward
    stability_temperature = 0.3  # Adjusted to increase impact
    stability_reward = torch.exp(-stability_temperature * torch.clamp(torch.norm(ang_velocity, p=2, dim=-1), max=2.0))  # Penalize excess angular movement
