...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.potentials, self.prev_potentials, self.up_vec, self.heading_vec, self.episode_length)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor, episode_length: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants
    time_penalty_scale = torch.tensor(0.001, device=root_states.device)
    ...
    # Normalize and transform reward components
    forward_vel_temperature = 0.2  # Increased temperature for forward velocity
    upright_temperature = 0.2      # Increased temperature for upright posture
    ...
    # Time penalty to encourage shorter episode lengths for efficiency
    time_penalty = -episode_length * time_penalty_scale

    # Total reward
    reward = forward_vel_reward_transformed + upright_reward_transformed + time_penalty
