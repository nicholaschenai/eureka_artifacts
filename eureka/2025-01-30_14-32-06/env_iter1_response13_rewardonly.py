...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_vel_scale = 0.5  # Adjusting the scale
    forward_vel_reward = forward_vel * forward_vel_scale
    ...
    # Angular velocity penalty to promote stability
    ang_vel_penalty = torch.norm(ang_velocity, p=2, dim=-1) * 0.05

    # Normalizing and transforming the reward components
    forward_vel_temperature = 0.15  # Adjusted temperature for forward velocity
    upright_temperature = 1.0       # Increased temperature for upright posture
    ...
    reward = forward_vel_reward_transformed + upright_reward_transformed - ang_vel_penalty
