...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # New Scaling for Forward Velocity Reward
    forward_vel_temperature = 0.1  # Reduced scaling factor
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Reformulated Upright Reward: Penalize significant deviation from uprightness
    ...
    upright_penalty_temperature = 0.5  # Introduction of a penalty scaling
    upright_reward = torch.exp(upright_penalty_temperature * (dot_prod_up - 1.0))

    # Modified Stability Reward for smaller transitions
    ang_velocity = root_states[:, 10:13]
    smoothness_temperature = 0.5  # Increased to emphasize smoother transitions
    stability_reward = torch.exp(-smoothness_temperature * torch.norm(ang_velocity, p=2, dim=-1))

    # Total reward with adjusted weightings
    total_reward = 0.7 * forward_vel_reward + 0.15 * upright_reward + 0.15 * stability_reward
