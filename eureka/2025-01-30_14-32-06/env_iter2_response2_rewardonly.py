...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Adjusted Reward for forward velocity with enhanced sensitivity
    forward_vel_temperature = 0.1  # Increased temperature
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Revised Upright reward component for greater sensitivity
    ...
    upright_temperature = 2.0  # Further increased temperature to boost variability
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 1.0))

    # Total reward is a weighted sum of the components with new weights
    total_reward = 1.5 * forward_vel_reward + 1.0 * upright_reward
