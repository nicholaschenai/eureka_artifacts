...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Adjust Forward Velocity Reward
    forward_vel_temperature = 0.4  # Increase emphasis
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Adjust Upright Reward
    ...
    # Increase sensitivity by squared penalty
    upright_temperature = 0.2  # Slightly increase influence
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 1.0)**2) - 1.0
    
    # Adjust Stability Reward
    smoothness_temperature = -0.2  # Boost meaningfulness
    stability_reward = torch.exp(smoothness_temperature * torch.norm(ang_velocity, p=2, dim=-1))

    # Total reward with adjusted weightings
    total_reward = 1.0 * forward_vel_reward + 0.3 * upright_reward + 0.5 * stability_reward
