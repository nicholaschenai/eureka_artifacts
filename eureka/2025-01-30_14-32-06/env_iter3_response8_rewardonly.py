...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_vel_temperature = 0.2  # Adjusted for better emphasis with minor changes
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Simplified Upright penalty formulation
    ...
    upright_penalty = torch.where(dot_prod_up < 0.95, 1.0 - dot_prod_up, torch.tensor(0.0, device=root_states.device))

    # Improved Stability reward to encourage smoother motion
    ang_velocity = root_states[:, 10:13]
    stability_temperature = 0.5  # Increased for greater differentiation between stable and unstable
    ...
    total_reward = 0.5 * forward_vel_reward - 0.1 * upright_penalty + 0.4 * stability_reward
