...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # More aggressive Reward for forward velocity
    forward_vel_temperature = 0.2  # Further increase in temperature
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Rewritten Upright reward component
    ...
    upright_temperature = 0.1  # Reduced impact significantly
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 1.0))

    # Enhanced Stability Reward for more appeal
    ang_velocity = root_states[:, 10:13]
    stability_temperature = 0.5  # Amplified temperature for rapid feedback
    ...
    total_reward = 1.5 * forward_vel_reward + 0.1 * upright_reward + 0.4 * stability_reward
