...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_vel_temperature = 0.1  # Adjust temperature for forward velocity to be more sensitive
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # New formulation for Upright reward component with more informative scaling
    ...
    # Enhance upright reward
    upright_temperature = 2.0  # Increase temperature
    upright_reward = torch.exp(dot_prod_up * upright_temperature) - 1.0

    # Total reward is a weighted sum of the components
    total_reward = 1.5 * forward_vel_reward + 0.5 * upright_reward
