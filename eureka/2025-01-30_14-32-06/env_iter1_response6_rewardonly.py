...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Re-scale forward velocity reward
    ...
    # Redefine upright reward component to penalize deviation from vertical
    ...
    upright_transformed = torch.exp(10 * (upright_reward - 1))  # Stronger penalty for non-upright posture

    # Total reward
    reward = forward_vel_reward_transformed + upright_transformed
