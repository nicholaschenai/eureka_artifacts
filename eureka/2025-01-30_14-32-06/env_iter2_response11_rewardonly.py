...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Adjusted Reward for forward velocity with more emphasis
    forward_vel_temperature = 0.1  # Slight increase to enhance the reward response
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Revised Upright reward component focused on stability and exploration
    ...
    # Incorporate a small transformation to ensure some reward without dominating
    upright_temperature = 0.1  # Lowered temperature to reduce its dominating effect
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 1.0))

    # Total reward is a weighted sum of the components
    total_reward = 2.0 * forward_vel_reward + 0.5 * upright_reward  # Emphasizing forward velocity
