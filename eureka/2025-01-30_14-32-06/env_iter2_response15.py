...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_vel_temp = 0.1  # Adjusted temperature for forward velocity to increase responsiveness
    forward_vel_reward = torch.exp(forward_vel_temp * forward_vel) - 1.0
    ...
    # Re-adjusted scaling and transformation
    upright_temp = 1.0  # Increased temperature for more influence
    upright_reward = (torch.exp(upright_temp * (dot_prod_up - 1.0)) - 1.0) * 0.5  # Scaled down contribution

    # Total reward is a balance of forward velocity and staying upright
    total_reward = 1.2 * forward_vel_reward + upright_reward
