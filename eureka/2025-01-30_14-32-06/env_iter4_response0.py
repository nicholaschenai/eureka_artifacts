...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Reward for forwarding velocity
    forward_vel_temperature = 0.2  
    forward_vel_reward = torch.exp(forward_vel_temperature * forward_vel) - 1.0

    # Adjust Upright Reward to consider the ant's broad stability and posture
    ...
    upright_temperature = 0.5  # Increase temperature to broaden its effect
    upright_reward = upright_temperature * torch.clamp(dot_prod_up, -0.5, 0.5)
    
    # Rescaled Stability Reward further applying temperature transformation 
    ang_velocity = root_states[:, 10:13]
    stability_temperature = 0.1  # Maintain low temperature to encourage smooth movement
    stability_reward = torch.exp(-stability_temperature * torch.norm(ang_velocity, p=2, dim=-1)) - 0.5

    # Total reward recalibration
    total_reward = 1.0 * forward_vel_reward + 0.3 * upright_reward + 0.4 * stability_reward
    ...
    return total_reward.clamp(min=0.0), reward_dict
