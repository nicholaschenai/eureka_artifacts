...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Forward Velocity Reward with scaled temperature
    forward_vel_temperature = 0.1  # Scaled down to reduce dominance
    forward_vel_reward = (torch.exp(forward_vel_temperature * forward_vel) - 1.0) * 0.8

    # Re-defined Upright Reward with increased influence
    ...
    upright_temperature = 1.0  # Increased to make a more noticeable impact
    upright_reward = torch.exp(upright_temperature * (dot_prod_up - 0.9)) - 1.0

    # Adjusted Stability Reward with higher impact
    ang_velocity = root_states[:, 10:13]
    stability_temperature = 0.5  # Increased to emphasize influence
    stability_reward = torch.exp(-stability_temperature * torch.norm(ang_velocity, p=2, dim=-1))

    # Total reward with adjusted weightings
    total_reward = 1.0 * forward_vel_reward + 0.5 * upright_reward + 0.5 * stability_reward
