...
def compute_reward(root_states: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Revised scale for forward velocity reward to further improve agent's speed optimization
    forward_velocity_scale = 2.0
    forward_velocity_reward = forward_velocity_scale * forward_velocity

    # Re-calculate energy penalty
    energy_penalty = torch.sum(actions**2, dim=-1)

    # Adjust the energy temperature for stronger influence on policy learning
    energy_temp = 1.5
    energy_penalty_scaled = -energy_temp * energy_penalty

    # Compute total reward, accounting for appropriate trade-offs
    overall_temp = 0.1
