...
def compute_reward(
    ...
    # Enhanced Open Reward
    temp_open = 2.0  # Increased to strongly encourage opening
    open_reward = torch.exp(temp_open * torch.clip(cabinet_dof_pos[:, 3], min=0.0, max=1.0))
    open_reward = torch.clamp(open_reward, min=0.0, max=1.0) # Ensure it's bounded

    # Improved Completion Bonus
    temp_completion = 2.0
    ...
    completion_bonus = torch.exp(temp_completion * completion_bonus)
