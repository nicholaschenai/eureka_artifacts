...
def compute_reward(franka_grasp_pos: torch.Tensor, drawer_grasp_pos: torch.Tensor, cabinet_dof_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Improved open reward
    open_pos_factor = torch.clip(cabinet_dof_pos[:, 3], min=0.0, max=1.0)
    temp_open = 1.5
    open_reward = torch.tanh(temp_open * open_pos_factor)

    # Re-evaluating task success recognition
    task_score_temp = 2.0 
    task_score = torch.where(cabinet_dof_pos[:, 3] > 0.8, torch.tensor(1.0, device=cabinet_dof_pos.device), torch.tensor(0.0, device=cabinet_dof_pos.device))
    task_score = torch.tanh(task_score_temp * task_score)

    # Total reward including potentially the sum or something similar
    weight_distance = 0.2
    weight_open = 0.5
    weight_task_score = 0.3
    total_reward = weight_distance * distance_reward + weight_open * open_reward + weight_task_score * task_score
