...
def compute_reward(root_states: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Enhance the forward reward scaling for better rewards with fast speeds
    max_speed = 15.0
    forward_reward = forward_velocity.clamp(min=0) / max_speed

    # Re-implement sideways penalty to ensure effective deterrence for non-forward directions
    sideways_velocity = torch.norm(velocity[:, 1:3], p=2, dim=-1)
    optimized_sideways_penalty = -1.0 * sideways_velocity / max_speed  # Greater negative weight

    # Adjust temperature values to optimize transformations and overall reward
    ...
    # Summing component rewards to form total reward
